...
async def amireallyalive(event):
    ...
    start = datetime.now()
    jmthonevent = await edit_or_reply(event, "**-ثواني**")
    end = datetime.now()
    ms = (end - start).microseconds / 1000
    _, check_sgnirts = check_data_base_heal_th()
    EMOJI = gvarstatus("ALIVE_EMOJI") or "  - "
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "**سورس جاك شغال**"
...
def jmthonalive_text():
    EMOJI = gvarstatus("ALIVE_EMOJI") or "  ✥ "
    jmthon_caption = "**سورس جاك يعمل بنجاح**\n"
    jmthon_caption += f"**{EMOJI} اصدار التيليثون :** `{version.__version__}\n`"
    jmthon_caption += f"**{EMOJI} اصدار جاك :** `{jmthonversion}`\n"
...
ROZ_PIC = "https://telegra.ph//file/17c7111c3d88b188b9dcc.jpg"
RAZAN = Config.TG_BOT_USERNAME
ROZ_T = (
    f"**⌯︙بوت جاك يعمل بنجاح 🤍،**\n"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("السورس") and event.query.user_id == sbb_b.uid:
            buttons = [
                [
                    Button.url("قنـاة السـورس", "https://t.me/kbbbd"),
                    Button.url("المطـور", "https://t.me/jbbbbf"),
                ]
            ]
